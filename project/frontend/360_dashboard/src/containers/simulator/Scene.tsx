...
class Scene extends Component {
  ...
  constructor(props: any) {
    ...
    let _trafficLight_binding_data = new Array<Array<{section: number;id: number}>>();
    ...
    const _inter = this.roadIntersection.updateLane();
  ...
  drawRoad=() => {
    ...
    const _sections = this.roadIntersection.getRoadSections();
    const _startBlinkTime = 10;
    for (let i = 0; i < _sections.length; ++i) {
      const _lane_in = _sections[i].getLaneIn();
      const _lane_out = _sections[i].getLaneOut();
      let _color = 0xffffff;
      for (let j = 0; j < _lane_in.length; ++j) {
        const _lane = _lane_in[j];
        const _cd = this.roadIntersection.getTrafficLightCD(_lane.getTrafficLightId());

        const _light_state: string = this.roadIntersection.getLaneState(_lane.getRoadSectionId(), j);
      ...
      for (let j = 0; j < _lane_out.length; ++j) {
        const _lane = _lane_out[j];
        // Sets the color of the traffic lights depending on the status
        const _color2 = parseInt(this.getColor('skeye_blue'), 16);

        var _direction: vec2 = ts.tsNormalize(_lane.getHead().minus(_lane.getTail()));
        var _division: number = ts.tsLength(_lane.getHead().minus(_lane.getTail())) / (this.lane_w * 0.4) + 1;
        for (var k = 1; k < _division; ++k) {
  ...
  async getNumberOfCars() {
    const data = new DataFromCamera();
    const rawData = await data.getDataFromCamera() || '';
    const numberCars = await data.getNumberOfCars(rawData);
  ...
  drawTriangle(topVertex: vec2, height: number, width: number, direction: vec2, color: number, isHollow?: boolean) {
    ...
    const _direction = ts.tsNormalize(direction);
    let _isHollow = false;
    if (isHollow !== undefined) {
      _isHollow = isHollow;
    }

    const _bottom = topVertex.minus(_direction.multiply(height));
    const _direction_perpendicular = ts.tsRotateByOrigin(_direction, Math.PI / 2);
    const _vertices = new Array<vec2>();
