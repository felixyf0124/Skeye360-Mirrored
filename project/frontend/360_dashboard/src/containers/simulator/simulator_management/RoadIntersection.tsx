...
export default class RoadIntersection {
    ...
    addNewVehicle(lane_id: number, section_id: number, speed: number, vehicle_id?: number, position?: vec2) {
      ...
      if (_lane_from.getObjIndex(_obj_v.getId()) > 0) {
        ...
        const _dis1 = ts.tsLength(_front_pos.minus(_lane_from.getHead()));
        const _dis2 = ts.tsLength(_lane_from.getTail().minus(_lane_from.getHead()));
        if (_dis1 < _dis2) {
    ...
    bindTrafficLight(trafficLight: TrafficLight) {
      ...
      for (let i = 0; i < _toBeBound.length; ++i) {
        const _index = this.getRoadSectionIndex(_toBeBound[i].section);
    ...
    updateLane() {
      ...
      const _intersections = new Array<Array<vec2>>();
      for (let i = 0; i < this.roadSections.length; ++i) {
        // check left side
        ...
        let _edge_right_L_sec = ts.line(_lane_right_L_sec.getHead(), _lane_right_L_sec.getTail());

        let _lane_dir = ts.tsNormalize(_lane_right_L_sec.getHead().minus(_lane_right_L_sec.getTail()));
        let _perpendicular_unit_vec = ts.tsRotateByOrigin(_lane_dir, Math.PI / 2);
        ...
        let _edge_left_R_sec = ts.line(_lane_left_R_sec.getHead(), _lane_left_R_sec.getTail());
    ...
    resortRoadSections() {
      const _resort = new Array<{index: number;angle: number}>();

      for (let i = 0; i < this.roadSections.length; ++i) {
        const _vec = this.roadSections[i].getTail().minus(this.roadSections[i].getHead());
        _vec.y *= -1;
        let _ang = ts.getAngleOfVec(_vec) / Math.PI * 180;
        if (_vec.x < 0) {
          _ang += 180;
        }
        _ang = (_ang + 360) % 360;
    ...
    resortTrafficLightQueue() {
      const _resort = new Array<number>();
      for (let i = 0; i < this.roadSections.length; ++i) {
      ...
      const _sortedQueue = new Array<TrafficLight>();
      for (let i = 0; i < _resort.length; ++i) {
      ...
      for (let i = 0; i < this.vehicles.length; ++i) {
        if (this.vehicles[i].getIsInTransition()) {
          if (this.vehicles[i].getAtPathSection() === 1) {
            let _section_id = this.vehicles[i].getRoadSectionId();
            let _lane_id = this.vehicles[i].getLaneId();
