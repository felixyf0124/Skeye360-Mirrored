...
class DBConnection:
    ...
    def insert_one_count(self, collection, direction, count, date_time, count_type, intersection_id_id):
        ...
        new_count = {
            "id": (self.find_latest_id(collection) + 1),
    ...
    def insert_counts(self, collection, count_list):
        """This method is to make a post query and insert manny count objects to database"""
        my_db = self._connect_mongo()
        my_db[collection].insert_many(count_list)
        LOGGER.info("Sending many counts to db")
    ...
    def find_latest_id(self, collection):
