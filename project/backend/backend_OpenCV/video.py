...
ap.add_argument('-c', '--config', 
                help = 'path to yolo config file', default= path+'yolov3-tiny.cfg')
ap.add_argument('-w', '--weights', 
                help = 'path to yolo pre-trained weights', default= path+'yolov3-tiny.weights')
ap.add_argument('-cl', '--classes', 
                help = 'path to text file containing class names',default= path+'coco.names')
args = ap.parse_args()
...
while cv2.waitKey(1) < 0:
    ...
    if(count > 6):
        print("WARNING: # of people is now", count ,"more than a limitation of 6.")
